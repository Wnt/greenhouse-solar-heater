...
- `playground/js/sim-bootstrap.js` → pure fast-forward helper. Exports `bootstrapSimulation({model, controller, durationSeconds, dt, getEnv})` which advances a `ThermalModel` in place and returns the recorded points + transition log entries. Used on deploys without live mode (e.g. GitHub Pages) to pre-roll 12 h of history into the dashboard before the user sees it. Unit-tested by `tests/sim-bootstrap.test.js`. The auto-bootstrap + auto-start path is wired in `main.js` via `bootstrapAndAutoStartSimulation()` and runs when `isLiveCapable === false`. The `window.__simulateGitHubPagesDeploy` flag is a Playwright-only test hatch that flips `isLiveCapable` so the e2e suite (`tests/e2e/auto-bootstrap.spec.js`) can exercise the GH Pages code path on localhost.
