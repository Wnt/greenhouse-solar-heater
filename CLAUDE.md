...
- **Two control paths.** WiFi (192.168.30.0/24) carries server/VPN/MQTT and server→hub management (sensor discovery/apply); the Pro devices additionally share an **isolated Ethernet switch** (no upstream, no DHCP) with **static IPs on 192.168.31.0/24** for the 4PM's command + polling paths. Valve commands AND sensor polls try the wire first, WiFi second (`VALVES`/`pollSensor` in `shelly/control.js` — the wired address is derived as `192.168.31.<WiFi last octet>`); `deploy.sh` provisions the static eth IPs (`ETH_*` in `devices.conf`). The wired subnet is deliberately distinct from the WiFi /24 — two interfaces in one subnet would misroute LAN traffic into the island. Heating autonomy survives a full WiFi outage; only telemetry/app/deploys need WiFi. See `system.yaml` `shelly_components.networking`.
