...
const CAPS = {
  ...
  STATE_BYTES: 760,               // JSON.stringify(state).length peak — includes transient transition fields (opening[], pending_closes[], manual_override{}) plus the published flags object (collectors_drained / emergency_heating_active / greenhouse_fan_cooling_active). Now deterministic: the 24 h sim uses a seeded PRNG. Previously 720 (peak 699 B); bumped 2026-05-01 to 760 after surfacing greenhouse_fan_cooling_active in the snapshot so the System Logs UI can render fan-cool overlay transitions (peak 737 + ~23 B headroom).
