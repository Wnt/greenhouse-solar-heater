...
const CAPS = {
  ...
  RUNTIME_PROXY_PEAK: 49600,      // re-calibrated 2026-06-25 after the #262 degraded-WiFi LOCAL HTTP hardening (sensor staleness cache + the valve-actuation timeout/bounded-retry/post-open-verify path + the valveCycleInFlight guard in control.js; the cost is mostly the larger setValve state machine; measured 49092 + ~508 B margin). The added bytecode is steady-state, NOT the transition-time JSON transient that caused the 2026-06 OOM — the per-emit telemetry spike is unchanged (Epic #254 moved it server-side). Still far under the 65535-byte Script.PutCode device hard limit (DEPLOYED_BYTES). Prior: 48800 (re-calibrated 2026-05-03 after introducing the user-tunable `tu` thresholds: TUNING_KEYS map + applyTuning helper in control-logic.js, plus `tu: {}` on the on-device deviceConfig literal; cumulative cost across 8 short-key entries; measured 48012 + ~788 B margin). Earlier: 48000 (measured 47199, set 2026-05-03 after adding the per-tick `held` diagnostic); 46000 (measured 45419, set 2026-05-01 after adding the greenhouse_fan_cooling overlay); 44700 (measured 44126, set 2026-04-23 after adding solarStallBypassDelta + mean-tank stall metric); 43800 (measured 43256, set 2026-04-20 after adding __test_setTemps / __test_controlTick hooks for scheduler-stack-fuzz automated-freeze coverage). Hooks are stripped at runtime by the __TEST_HARNESS guard — this only affects the in-test measurement.
  STATE_BYTES: 600,               // greenhouse/state/min payload peak (Epic #254 #258). The device now emits the MINIMAL decision-state payload (buildMinPayload) — valves/actuators/controls_enabled/manual_override are reassembled server-side, so they no longer count toward the device-emit transient. Lowered 2026-06-24 from 820 → 600: the 24 h sim peak dropped from 779 B (full payload) to 512 B (minimal: ts/mode/transitioning/transition_step/temps/flags/opening/queued_opens/pending_closes/cause/reason/eval_reason/held), measured deterministic via the seeded PRNG; +88 B margin. Pre-#254 history: 820 (held in every snapshot, peak 779); 760 (greenhouse_fan_cooling overlay); 720 before that.
