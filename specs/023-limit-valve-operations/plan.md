...
**Constraints**: Shelly ES5-only device scripts (no `const`/`let`, arrow functions, classes, template literals). Shelly resource limits: ~~16 KB max script size~~ *(the 16 KB figure in earlier drafts is stale — current `shelly/control.js` is already ~21.6 KB and `shelly/control-logic.js` is ~11.8 KB, so the real per-slot limit on the deployed firmware is higher than 16 KB and has not been pinned to a specific number. Task T050a records the post-feature byte delta; no pre-upload guard is added — we rely on the device to reject oversized scripts and will document the observed limit only if deploy fails.)*; 5 timers; up to 5 concurrent HTTP calls (the SCHEDULE-loop actuation path uses a bounded worker pool with N = 4 to keep one slot free for telemetry — see T050b); 5 event subscriptions (this feature adds none). Control-loop polling interval is 30 s — the staged-opening state machine uses its own short-lived `state.transitionTimer` rather than waiting for the next control tick; timer-budget analysis is in research.md R2. KVS 256-byte limit per key applies if any of the new constants ever move into device config (default is in-code constants, no KVS bump).
