...
const displacementOut = path.resolve('playground/assets/liquid-glass-displacement.png');
const specularOut = path.resolve('playground/assets/liquid-glass-specular.png');
const metaOut = path.resolve('playground/assets/liquid-glass.json');

const browser = await chromium.launch();
const page = await browser.newPage();

const result = await page.evaluate(async (cfg) => {
  const { width, height, radius, bezelWidth, thickness, n, light, specularPower } = cfg;
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (d < 0 && -d < bezelWidth) {
        ...
        // Specular rim: steep surface + normal tilted toward light
        // inward 2D direction × 2D light direction; scaled by sin(θ₁) so rim is strongest where surface is steepest
...
console.log(`maxDisplacement = ${result.maxMag.toFixed(2)} px  (use as <feDisplacementMap scale>)`);
