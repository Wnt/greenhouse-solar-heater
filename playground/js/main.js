...
// The balance card on the Status view needs a fixed 48 h slice that
// doesn't shrink when the user flips the history graph's 1 H/6 H/… pill.
// We pull it separately and apply live state frames on top so the numbers
// stay fresh without hammering /api/history on every WebSocket message.
//
// balanceHistory holds the raw server response; balanceLivePoints is an
// incremental tail of points appended from live WS frames (rate-limited
// to one per 5 min, matching the database aggregation cadence).
...
// Time formatting + cause/reason labels moved to ./main/time-format.js
