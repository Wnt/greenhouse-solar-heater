...
**Interaction with the valves-first drain-exit sequence.** `transitionTo()` (post the 2026-04-19 drain-exit work) already branches on `state.transitionFromMode === ACTIVE_DRAIN` to use the valves-first + `DRAIN_EXIT_PUMP_RUN_MS` (20 s) path. If the user forced `mo.fm = "AD"` and then exits override, our `transitionTo(buildIdleTransitionResult())` call hits that same branch automatically — no extra work in this spec. User-visible consequence: the pump keeps running for ~20 s after valves close, so the Status view's chip should reflect `ACTIVE_DRAIN → IDLE` ordering correctly (the existing `transition_step` snapshot already does this). UI note: when TTL expires mid-forced-AD, the countdown hits 0 but the state chip will show the drain-exit valves-closing step for up to ~21 s before IDLE — the "Override expired" copy already waits for the next state snapshot, so no extra UI work is needed.
...
- **`tests/override-forced-mode.test.js`** (new): reuse the `createOrderingRuntime` harness from `tests/shelly-transition.test.js` and the `__TEST_HARNESS` / `Shelly.__test_driveTransition` hook shipped in the 2026-04-19 drain-exit work. Tests: (a) enter override, set `mo.fm = "SC"`, assert valves snap to `MODE_VALVES.SOLAR_CHARGING` and pump on; (b) switch to `"AD"`, assert staged transition runs and min-duration is ignored; (c) exit override from `SC`, assert `transitionTo(IDLE)` uses the default pump-first path; (d) **exit override from `AD`**, assert `transitionTo(IDLE)` uses the valves-first + `DRAIN_EXIT_PUMP_RUN_MS` path — regression guard against anyone "simplifying" the exit to always go pump-first; (e) TTL expiry triggers the same IDLE transition without a user action.
