# Managed MySQL High availability
...
In the event of a MySQL® standby node failure, the primary node continues normal operation, maintaining service levels for client applications. Upon readiness, the replacement standby node synchronizes with the primary, initiating real-time replication until stability is restored.

If the failed node is a MySQL primary node, failover decisions are made using information gathered from the monitoring infrastructure and the standby node. Subsequently, the standby node is elevated to the role of primary, promptly resuming client service. An automatic process schedules a replacement node to serve as the new standby.
