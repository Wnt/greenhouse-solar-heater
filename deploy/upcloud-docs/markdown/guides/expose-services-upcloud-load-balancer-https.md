...
Managed Kubernetes has a built-in [Cloud Controller Manager](https://github.com/UpCloudLtd/upcloud-cloud-controller-manager) that interfaces with the UpCloud API and manages other cloud resources, such as load balancers.

Note that if your Managed Load Balancer is managed by the Managed Kubernetes cluster, all Load Balancer configuration changes should be made through object annotations. See [below](/docs/guides/expose-services-upcloud-load-balancer-https#customising-load-balancer-configuration.md) for details and examples. **All manual modifications done to the Load Balancer through Hub or API are overwritten.**
...
- Managed Load Balancer *will not* be deleted if you just delete the whole Managed Kubernetes cluster. You either have to delete all service objects before deleting the cluster, or manually delete all managed Load Balancers after deleting the cluster (via the [UpCloud Control Panel](https://hub.upcloud.com) or [upctl](/docs/tooling/cli.md).
...
If the frontend `mode` set to `http` and port set to `443` & `tls_config` is not set, then tls\_config for this forntend
...
- Create a service with a production-grade load balancer plan with a custom name [lb-svc-production-plan.yaml](https://github.com/UpCloudLtd/uks-instructions/blob/main/ccm/lb-svc-production-plan.yaml)
- Create a service with HTTP redirect to HTTPS and configured forwarded-for headers to applications can get and log actual IP addresses of connecting clients: [lb-svc-ip-forwarded-for.yaml](https://github.com/UpCloudLtd/uks-instructions/blob/main/ccm/lb-svc-ip-forwarded-for.yaml)
- Create a service with [custom network configuration](/docs/guides/customise-load-balancer-network-configuration.md)
