...
Upcloud's CSI driver only supports offline expansion so the application needs to be scaled down before operation.
...
This document uses a `mariadb` deployment from the guide on [Persistent Volumes](/docs/guides/using-csi-driver-persistent-volumes#deploy-mariadb-using-persistent-volume.md) as an example.
