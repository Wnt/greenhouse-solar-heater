# Customising Load Balancer's network configuration
...
- list **must** include cluster's private SDN network
...
### Expose services through TCP load balancing

If the service should handle raw HTTP requests or TLS termination is done on the backend service,
you will need to modify the UpCloud Load Balancer to use TCP load balancing. With our example:
...
Note that the name parameters in the `frontends` list has to match the ports that are being exposed by the service. If you would have both "http" and "https" ports exposed over TCP load balancing, the configuration looks like this:
