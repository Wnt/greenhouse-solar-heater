# Importing an existing block storage to your Managed Kubernetes cluster

**This should be done very carefully and at your own risk, as there is a possibility of data loss if something goes wrong.**

Importing a block storage volume as a Persistent Volume Claim to your Managed Kubernetes cluster requires following steps:
...
- `kubectl` installed
- Block storage volume in the same zone as your Managed Kubernetes cluster
- Backups taken prior to starting the operation
- Main account access to UpCloud's Hub (hub.upcloud.com)

## Grant device permissions

Access your Managed Kubernetes cluster. CSI driver is run using sub-account credentials.
...
Go to the permission section of the UpCloud Control Panel and grant CSI sub-account for permission to access the storage volume you want to use in Managed Kubernetes.
