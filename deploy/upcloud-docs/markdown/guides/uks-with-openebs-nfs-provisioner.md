# How to implement self-managed NFS support with OpenEBS on Managed Kubernetes

[UpCloud Managed Kubernetes](https://upcloud.com/products/managed-kubernetes/) supports Read-Write-Once (RWO) Persistent Volume out of the box. However, for use cases where Read-Write-Many (RWX) Persistent Volumes are needed, OpenEBS provides a good alternative. [OpenEBS with NFS Provisioner](https://github.com/openebs/dynamic-nfs-provisioner/blob/develop/docs/intro.md) can utilize the existing RWO volumes, and expose them through automatically created NFS server as RWX volumes. This guide goes through the implementation steps required to make OpenEBS functional on top of Managed Kubernetes.

## Prerequisites

Start by building your Managed Kubernetes cluster. You will need SSH access to the worker nodes, so remember to add your SSH keys to the worker nodes!

![Add SSH key](img/image.png)

In addition to a Managed Kubernetes cluster, you will need `helm` to be installed and configured.

The Managed Kubernetes worker nodes need to be prepared before OpenEBS can provision RWX compatible Persistent Volumes. OpenEBS requires that the nodes are configured with an NFS client. Managed Kubernetes worker nodes do not have NFS client built-in for now, so they need to be installed manually. Note that the worker node image might be recreated in certain circumstances, like in node pool modifications, and the NFS client will need to be reinstalled.
...
The last thing to do is the create a Storage Class for RWX volumes that configures the NFS Provisioner and adds the standard Managed Kubernetes Storage Class `upcloud-block-storage-maxiops` as the backend Storage Class which is used to provision the underlying RWO volumes:
