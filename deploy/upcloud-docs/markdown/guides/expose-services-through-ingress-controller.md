# Using Ingress NGINX Controller to expose Kubernetes deployments and services

In this guide we will create a Managed Kubernetes cluster, deploy an example app and expose it with [Ingress NGINX Controller](https://kubernetes.github.io/ingress-nginx/). Last optional step will be to keep external DNS records in sync for public access.
...
## (Optional) Keep external DNS records in sync with ExternalDNS and CloudFlare DNS service

In this example, we are using [CloudFlare DNS service](https://www.cloudflare.com/application-services/products/dns/) as an example for managing the DNS records. This setup works alongside [ExternalDNS](https://github.com/kubernetes-sigs/external-dns), which automatically synchronises your Kubernetes services and ingresses with external DNS providers.

Modify the existing Helm release for Ingress NGINX controller to match the following in the Terraform configuration: (replace `example.com` with your hostname)
