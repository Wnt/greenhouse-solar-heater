# How to get started using the UpCloud API with HTTP Basic authentication

This guide covers how to authenticate with the UpCloud API using a subaccount's username and password over HTTP Basic authentication. For new integrations we recommend using [API tokens](/docs/guides/getting-started-upcloud-api.md) instead, as they have configurable expiration dates, can be restricted to specific IP addresses, and can be revoked individually if compromised. Username and password authentication remains fully supported and is documented here for tools and workflows that still rely on it.
...
## Creating an API subaccount

The first step is to create a separate API user. You can do this at the [UpCloud Control Panel](https://hub.upcloud.com/people) under **People → Accounts**. Your API account name and password are very much comparable to an API ID and key pair with the added benefit of being able to set them yourself.

Create a new subaccount for the API access by clicking the *Create subaccount* button.

![People — Accounts](../getting-started-upcloud-api/img/image.png)
...
![Create API account](../getting-started-upcloud-api/img/image-1.png)
...
Using a GET request, enter the API address <https://api.upcloud.com> with the API version number and the desired command to the request URL line, then select the Basic Auth option and set the subaccount's username and password. When you have filled in the request details, click the *Send* button to run the query.

![Postman GET Account](../getting-started-upcloud-api/img/image-2.png)

Postman GET Account

The reply will be shown in the bottom half of the Postman window under the *Body* tab.

The authentication method here is HTTP Basic authentication, where the `Authorization` header should contain your subaccount's username and password Base64 encoded. More precisely:
...
![Postman Basic Auth header](../getting-started-upcloud-api/img/image-3.png)
...
The API can be accessed using any language that has proper HTTP libraries. The following snippet shows how you could take advantage of the UpCloud API using Python3 with Base64-encoded subaccount credentials.
