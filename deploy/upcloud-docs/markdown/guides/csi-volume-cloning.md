...
This document uses `mariadb` deployment from the guide on [Persistent Volumes](/docs/guides/using-csi-driver-persistent-volumes#deploy-mariadb-using-persistent-volume.md) as an example. To test this out, follow the steps from that guide and fill the database with test data (numbers).
